...
public static class VersionBumper {
    ...
    /// </summary>
    public static Version ComputeTargetVersion(Version? latestTag, Dictionary<string, Version> projectVersions, List<ProjectDiffResult> diffResults) {
        Version baseVersion = latestTag ?? new Version(0, 0, 0);

        // Determine bump from syntactic analysis
        bool hasBreaking = diffResults.Any(d => d.HasBreakingChanges);
        bool hasAnyChanges = diffResults.Any(d => d.HasBreakingChanges || d.HasNonBreakingChanges);
        ...
        Version maxProjectVersion = projectVersions.Values.DefaultIfEmpty(new Version(0, 0, 0))
            .Max()!;
